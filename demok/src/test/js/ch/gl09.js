...
void main() {
  ...
  vTexcoord = aTexcoord;
  vNormal = aNormal;
  vLight = uLight - (uLightMatrix * aPosition).xyz;
}
...
void main() {
  float brightness = dot(normalize(vLight), normalize(vNormal) );
  brightness = max(0.0, brightness);
  brightness = pow(brightness, 1.0) + pow(brightness, 100.0);
  brightness = brightness + uEnvLight;
...
*/
    ...
    !function() {
      // light some.
      var lightMat = mat4();
      var uLightMatrixLoc = gl.getUniformLocation(program, 'uLightMatrix');
      gl.uniformMatrix4fv(uLightMatrixLoc, false, lightMat);

      var uLightLoc = gl.getUniformLocation(program, 'uLight');
      var a = 0.8; 
      gl.uniform3fv(uLightLoc, [ -a, a, -a * 0.8 ]);

      var uEnvLightLoc = gl.getUniformLocation(program, 'uEnvLight');
      gl.uniform1f(uEnvLightLoc, 0.4);
    }();
    ...
    this.enterFrame = function(time) {
      ...
      gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(nrmArr), gl.STATIC_DRAW);
